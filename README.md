...
```bash
researchctl artifact register adapter_exchange --stage experiment_results \
...
验证成功后，Adapter 必须先把该 attempt 的首条 `accepted` 回执追加到 working exchange，并再次 `artifact register`；只有这条执行前 journal 成为不可变 revision 后，Core 外的 Adapter 才能消费绑定输入并产生实际副作用，后续观察不能与首条 `accepted` 在同一 revision 首次出现。后续 `running|succeeded|failed|cancelled|unknown` 观察以 `supersedes` 追加，相关 output/log 先登记为 artifact。所有 Gate-approved revisions 都必须出现在 operational inputs 中；外部发布的 ordered inputs 必须与获批 release package 精确相等且不得夹带额外 artifact，payload 也必须是 package 内的精确 revision。Gate 在执行期间重开时，已 journaled attempt 的晚到回执仍保留，但新的 `accepted` journal、派发或 retry 会被拒绝；`reconcile_before_retry` 下的 `unknown` 必须先核对同一 attempt。operation 到 stage、GateRef、effect class 与人类授权的唯一映射在 [`policy.adapter_authority`](skills/research/references/policy.yaml)，Core/Adapter/人的边界和非能力见 [ADR 0004](decisions/0004-adapter-authority-exchange.md)。该 exchange 是普通 registered artifact，state 不增加 operation、attempt 或 provider store。
