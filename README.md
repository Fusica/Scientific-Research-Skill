...
它不承诺自动产出“顶刊论文”，而是让研究判断、证据、实验和写作之间的关系清楚、可恢复、可审计。
...
旧版八个并列 Skill 已收敛为一个入口和六个按需加载的阶段 reference。旧版 `.planning/`、`project-overview.md` 和 YAML Gate state 不再参与运行时，也不会被重新创建。
...
Codex 当前没有为单个 command Hook 指定独立模型的原生接口。需要时可在研究项目的 `.codex/config.toml` 设置会话主模型和 reasoning effort；该设置同时影响普通对话、Skill 和停止前语义复核。v1 不从 Hook 额外调用模型 API。参见 [Codex Hooks](https://developers.openai.com/codex/hooks) 与 [Configuration reference](https://developers.openai.com/codex/config-reference)。
...
本地组合层采用 Apache-2.0。仓库保留 Claude Scholar、EvoSkills、Nature Skills 等经过许可审查的上游快照与来源记录；缺少仓库许可证的 agent-research-skills 仅做 clean-room 思想重构。上游材料不注册为运行时 Skill，具体边界见 [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md) 与 [upstreams.lock.yaml](upstreams.lock.yaml)。
